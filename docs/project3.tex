...
Machine learning is one of the most flexible and succesful methods for this, but it is often limited at answering the question \textit{how}, but not the \textit{why} of the effects.
...
In this project we infer a causal model as efficiently as possible and develop a work method to try to get the most out of interventional observation.
...
To consider whether the change was significant a student t-test was conducted to see whether the means were changed.
Furthermore, confidence intervals on correlation along with comparison of standard deviation gave insight in the effects of the different variables.
Barlett's test of equal variance was also used to determine if the change in variance was significant. 
...
As an additional strategy for structured inference, all 42 (the number of 2-permutations of 7 elements) possible edges in the grap were noted.
By eliminating the possibilites that were unlikely to have generated the data, a smaller subset of candidate edges were achieved.
...
\section{Discussion}
The approach of a requesting a higher number of  queries, each with few data points, had strengths and weaknesses.
...
It also had some drawbacks that resulted in some confusion.
...
During most of the inference, linear, additive effects were the framework for modelling the graph.
