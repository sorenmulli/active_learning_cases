...
\title{Causal Inference: The Data of 7 Intertwined Effects }
...
\begin{itemize}
	...
	\item Statistical tests: Student t test of means. Confidence interval on correlations. Comparing standard deviations.
	...
	\item Constructing hypotheses consistent with the data and trying loosely to apply counterfactuals to predict next query.
\end{itemize}
...
\paragraph{Cost}
\[
\underbrace{425}_{\text{\# points}} + \underbrace 9_ {\text{\# experiments}} \cdot 20 +\underbrace{70}_{\text{Guess}}=612
\]

\begin{figure}[H]
	\centering
	\includegraphics[width=.6\linewidth]{wronggraph}
	\caption{Wrong graph guess. Note that especially the role of node A is misunderstood. Three edges are correct and four are wrong. Two nodes (P and K) have three conected edges.}
	\label{fig:wronggraph}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=.6\linewidth]{correctgraph}
	\caption{Correct graph. The causal complexity is lower here than in the wrong graph with no nodes having three connections.}
	\label{fig:correctgraph}
\end{figure}

	
\section{Discussion}
The approach of more queries each with few data points had strengths and weaknesses. It allowed for a healthy process of estimating the expected result under each of the hypotheses that were viable at that time .
