...
\begin{abstract}
\noindent Optimization of neural network hyperparameters is a costly process usually pervaded by uncertainty. This report compares the abilities of Bayesian optimization with Gaussian processes to simple grid search in finding optimal hyperparameters for the fully-connected layers of the VGG16 image classification network. It is concluded that grid search performs slightly better than bayesian optimization, which could have to do with the low amount of opportunities explored. To have real implications on future papers, an experiment of bigger scale would have to be conducted to reap the true benefits of what BO has to offer.
